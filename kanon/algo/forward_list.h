...
class ForwardListBase 
  : protected NodeAllocator<T, Alloc>, protected HeaderAllocator<Alloc> {
private:
...
public: 
  ...
  // But I expose these.
  // Not standard required
  inline void push_front(Iterator node) {
    push_front(node.extract());
  }

  inline void push_back(Iterator node) {
    push_back(node.extract());
  }

  inline void insert_after(ConstIterator pos, Iterator node) {
  ...
  // Not standard required
  inline Iterator extract_front() noexcept {
    return Iterator(extract_front_node());
  }

  inline Iterator extract_after(ConstIterator pos) noexcept {
  ...
  Ref front() noexcept { assert(!empty()); return GET_LINKED_NODE_VALUE(header_->next); }
  ConstRef front() const noexcept { assert(!empty()); return GET_LINKED_NODE_VALUE(header_->next); }

  // Not standard required
  Ref back() noexcept { assert(!empty()); return GET_LINKED_NODE_VALUE(header_->prev); }
  ConstRef back() const noexcept { assert(!empty()); return GET_LINKED_NODE_VALUE(header_->prev); }
  ...
  Iterator search_before(ValueType const& val, ConstIterator pos) {
    return search_before([&val](ValueType const& value) { return value == val; }, pos);
  }
   ...
   */
  void merge(Self& list) { merge(list, [](ValueType const& x, ValueType const& y) { return x < y; }); }
  ...
  SizeType remove(ValueType const& val) { return remove_if([&val](ValueType const& value) { return value == val; }); }
  template<typename UnaryPred>
  ...
  // In C++20, the return type is modified to size_type(i.e. SizeType here).
  // It indicates the number of removed elements
  SizeType unique() { return unique([](ValueType const& x, ValueType const& y) { return x == y; }); }
  template<typename BinaryPred>
  ...
  void sort() { return sort([](ValueType const& x, ValueType const& y) { return x < y; }); }
  ...
  inline Node* extract_front_node() noexcept;
  inline Node* extract_after_node(ConstIterator pos) noexcept;
...
private:
  ...
  template<typename Dummy=zstl::enable_if_t<std::is_default_constructible<ValueType>::value>>
  void resize_impl(size_t n) { resize(n, ValueType{}); }
