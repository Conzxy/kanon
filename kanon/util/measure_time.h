...
#define TIME_INTERVAL              (CHRONO::high_resolution_clock::now() - begin_)
#define DURATION_CAST(timetype, d) (CHRONO::duration_cast<CHRONO::timetype>(d))
#define MEASURE_CLOCK              CHRONO::high_resolution_clock
...
class MeasureTime {
 public:
  ...
  explicit MeasureTime(StringView func_name,
                       TimeType ty = TimeType::nanoseconds)
    : func_name_{func_name}
    , begin_{MEASURE_CLOCK::now()}
    , time_t_{ty} {};

  void reset() { begin_ = MEASURE_CLOCK::now(); }

  ~MeasureTime()
  {
    switch (time_t_) {
      case TimeType::seconds:
        printTime(DURATION_CAST(seconds, TIME_INTERVAL));
        break;
      case TimeType::milliseconds:
        printTime(DURATION_CAST(milliseconds, TIME_INTERVAL));
        break;
      case TimeType::microseconds:
        printTime(DURATION_CAST(microseconds, TIME_INTERVAL));
        break;
      case TimeType::nanoseconds:
        printTime(DURATION_CAST(nanoseconds, TIME_INTERVAL));
        break;
    }
  }
                      ...
                      TimeType ty = TimeType::nanoseconds)
  {
    ...
    switch (ty) {
      case TimeType::seconds:
        return static_cast<count_t>(DURATION_CAST(seconds, interval).count());
        break;
      case TimeType::milliseconds:
      ...
      case TimeType::microseconds:
        return static_cast<count_t>(
            DURATION_CAST(microseconds, interval).count());
        break;
      case TimeType::nanoseconds:
        return static_cast<count_t>(
            DURATION_CAST(nanoseconds, interval).count());
        break;
    }
  }
...
KANON_INLINE void measureTime(Func &&func, StringView func_name, Args &&...args)
{
...
#define MEASURE_TIME(func, ...) measureTime(func, #func, ##__VA_ARGS__)
