...
TimeStamp Poller::Poll(int ms, ChannelVec &active_channels) KANON_NOEXCEPT
{
  ...
  auto ret = ::poll(pollfds_.data(), static_cast<nfds_t>(pollfds_.size()), ms);
...
void Poller::UpdateChannel(Channel *ch)
{
  ...
  if (it == channels_map_.end()) {
    ...
    channels_map_.emplace_hint(channels_map_.end(), ch->GetFd(), ch);
    ...
    pollfds_.emplace_back(
        std::move(new_pollfd)); // although std::move is meaningless since
                                // pollfd is POD type

    // channel index <==> pollfds size
    assert(pollfds_.size() > 0);
    ch->SetIndex(int(pollfds_.size() - 1));
  } else {
