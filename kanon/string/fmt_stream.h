...
class FmtStream {
 ...
 private:
  ...
  void SerializeOne(T arg) KANON_NOEXCEPT
  {
    stream_.appendInt(arg);
  }

  template <typename T, typename std::enable_if<
                            std::is_floating_point<T>::value, char>::type = 0>
  void SerializeOne(T arg) KANON_NOEXCEPT
  {
    stream_.appendFloat(static_cast<double>(arg));
  }
  ...
  void SerializeOne(std::string const &s) KANON_NOEXCEPT
  {
    stream_.Append(s.data(), s.size());
  }
  void SerializeOne(StringView sv) KANON_NOEXCEPT { stream_.Append(sv); }
...
TEMPLATE_OF_FMT_STREAM
template <typename Arg, typename... Args>
void FMT_STREAM_TEMPLATE::ParseAndSerializeSingle(StringView fmt, Arg arg,
                                                  Args... args)
{
  // Parse
  ...
  if (pos < fmt.size()) {
    ...
    KANON_ASSERT(false,
                 "The number of argument is greater than the % placeholder");
  }
}
