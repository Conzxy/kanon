...
static char const* 
GetRpcErrorString(RpcMessage::ErrorCode error) noexcept;

char const kanon::protobuf::rpc::krpc_tag[] = "krpc";

KRpcChannel::KRpcChannel() = default;

// Constructor does not recommended to do many thing except for initial work
KRpcChannel::KRpcChannel(TcpConnectionPtr const& conn)
  : id_(0)
  , codec_()
{
  SetConnection(conn);
...
void KRpcChannel::ErrorHandle(uint64_t id, ErrorCode error_code)
{
  RpcMessage message;
  message.set_id(id);
  ...
  TimeStamp receive_time)
{
  ...
  KANON_ASSERT(
    ...
    "The type and id field is required, the protobuf of peer must ensure them are setted"); 
  ...
  if (message->type() == RpcMessage::kResponse) {
    ...
    KANON_ASSERT(false, "RpcMessage Type Error occurred impossible, it is must be internal error");
  }    
}

void KRpcChannel::OnRpcMessageForResponse(
  TcpConnectionPtr const& conn,
  RpcMessagePtr message,
  TimeStamp stamp)
{
  ...
  if (!message->has_error()) {
    ...
    KANON_ASSERT(id <= id_, "The id in response should be not greater than the id_ maintained by client");
    ...
    KANON_ASSERT(
      it != call_results_.end(),
      "The call_result must be setted before the RpcMessage on_message Callback");
    ...
    KANON_ASSERT(call_result.response != nullptr, "response must be setted by client");
    ...
    const auto n = call_results_.erase(message->id());KANON_UNUSED(n);
    assert(n == 1);
    
    // Since this error is logic error(i.e. ensure it don't happened)
    LOG_FATAL << "Rpc error message from server: " << GetRpcErrorString(message->error());
  }
}

void KRpcChannel::OnRpcMessageForRequest(
  TcpConnectionPtr const& conn, 
  RpcMessagePtr message,
  TimeStamp stamp)
{
    ...
    if (it != services_->end()) {
      ...
      if (method) {
        ...
        if (request->ParseFromString(message->request())) { 
          ...
          // NewCallback() accept type should be perfect match, i.e. don't allow implicit cast
          // e.g.
          // void Foo(std::string);
...
void KRpcChannel::SetConnection(const TcpConnectionPtr &conn) noexcept
{
  conn_ = conn;

  // Forward to codec to process raw-format message
  conn_->SetMessageCallback(std::bind(
    &Codec::OnMessage, &codec_, _1, _2, _3));

  // Handle the protobuf-format RpcMessage(i.e. the payload after the codec_ parsing)
  codec_.SetMessageCallback(std::bind(
    &KRpcChannel::OnRpcMessage, this, _1, _2, _3));
}
